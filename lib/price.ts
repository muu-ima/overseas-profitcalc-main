...
/**
 * 最終価格を計算(VAT加算を含めるか選択)
 * @param priceGBP GBP価格
 * @param rate 為替レート(手数料込み)
 * @param includeVAT VATを含めるかどうか (デフォルトfalse)
 */
export function calcFinalPrice(
    ...
    const base = includeVAT ? applyVAT(priceGBP) :priceGBP;
    return convertToJPY(base, rate);
}